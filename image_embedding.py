...
# --- Helper Functions --- (Identical to process_optical_flow.py, but get_image_embedding will be used)
...
from sentence_transformers import SentenceTransformer # Needed for get_caption_and_embedding if it were used
...
def get_caption_and_embedding(frame_rgb_pil, current_cap_processor, current_cap_model, current_text_embed_model):
    # This function will not be called by image_embedding method for its core sampling logic
    ...
    # ... (rest of function, though not directly used by 'image_embedding' method during sampling)
    with torch.no_grad():
...
def split_and_sample_advanced(
    ...
    for clip_idx in range(num_clips):
        ...
        for current_frame_num_in_video in range(start_frame_idx, end_frame_idx):
            ...
            if method == 'optical_flow': # This won't be true for this script
            ...
            elif method in ['image_embedding', 'caption_embedding']: # This will be true for image_embedding
                ...
                try:
                    ...
                    elif method == 'caption_embedding': # This won't be true for this script
        ...
        if not processed_frame_info:
            ...
            elif method in ['image_embedding', 'caption_embedding']: # Will be true for this script
...
for method_config in SAMPLING_METHODS_CONFIG:
    ...
    try:
        if method_name == 'image_embedding': # This will be true
            ...
            print("Image embedding model (ViT) loaded.")
        # elif method_name == 'caption_embedding': # Not for this script
            # ...
    except Exception as e:
    ...
    num_sampled_frames = split_and_sample_advanced(
        ...
        # Pass None for captioning models as they are not loaded/needed for this method's sampling
        current_caption_processor=None, current_caption_model=None,
        current_text_embedding_model=None
    )
...
print(f"\n\n================ IMAGE EMBEDDING METHOD PROCESSED ================")