...
# --- Helper Functions --- (Identical to process_optical_flow.py, but get_caption_and_embedding will be used)
...
create_dummy_video_if_not_exists(VIDEO_PATH, CLIP_DURATION_SECONDS * (1 * 3 + 10))

from transformers import (
    AutoProcessor, AutoModel, AutoModelForCausalLM,
    LlavaForConditionalGeneration, LlavaProcessor
)
from sentence_transformers import SentenceTransformer # Needed for get_caption_and_embedding
...
def get_image_embedding(frame_rgb_pil, current_img_processor, current_img_model):
    # This function will not be called by caption_embedding method for its core sampling
...
# split_and_sample_advanced, extract_sections_xml, llava_text_generate
# are identical to process_optical_flow.py
def split_and_sample_advanced(
    ...
    if method == 'image_embedding' and (current_img_processor is None or current_img_model is None): # Not for this script
        print("Error: Image embedding model not loaded. Skipping sampling.")
        cap.release()
        return 0
    if method == 'caption_embedding' and \
       (current_caption_processor is None or current_caption_model is None or current_text_embedding_model is None): # Will be true
    ...
    for clip_idx in range(num_clips):
        ...
        for current_frame_num_in_video in range(start_frame_idx, end_frame_idx):
            ...
            if not (is_first_frame_to_sample or is_nth_frame_to_sample):
                if method == 'optical_flow': # Not for this script
            ...
            elif method in ['image_embedding', 'caption_embedding']: # This will be true
                frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                frame_pil = Image.fromarray(frame_rgb)
                try:
                    if method == 'image_embedding': # Not for this script
                    ...
                    elif method == 'caption_embedding': # Will be true
        ...
        if not processed_frame_info:
            ...
            if method == 'optical_flow': # Not for this script
            ...
            elif method in ['image_embedding', 'caption_embedding']: # Will be true
                if len(processed_frame_info) > 0:
                    ...
                    if embeddings.shape[0] > 0:
                        ...
                        if method == 'caption_embedding': # Will be true
                             print(f"  {method}: Selected frame {selected_frame_video_idx} ('{selected_item.get('caption','N/A')[:30]}...', Sim to mean: {similarity_score:.4f})")
                        else: # For image_embedding (not this script)
...
for method_config in SAMPLING_METHODS_CONFIG:
    ...
    print(f"\n\n================ PROCESSING METHOD: {method_name.upper()} (Caption Embedding) ================")

    # Stage 0: Load models for sampling
    torch.cuda.empty_cache()
    # temp_img_processor, temp_img_model = None, None # Not loaded for this method
    temp_caption_processor, temp_caption_model, temp_text_embedding_model = None, None, None
    try:
        # if method_name == 'image_embedding': # Not for this script
            # ...
        if method_name == 'caption_embedding': # Will be true
            ...
            print("Captioning (GIT) and text embedding models loaded.")
    except Exception as e:
    ...
    num_sampled_frames = split_and_sample_advanced(
        ...
        # Pass None for image embedding models
        current_img_processor=None, current_img_model=None,
    ...
    # (Identical to process_optical_flow.py)
    torch.cuda.empty_cache()
...
print(f"\n\n================ CAPTION EMBEDDING METHOD PROCESSED ================")